			...
			<div className="w-full lg:w-8/12 h-fit flex flex-col gap-1 justify-evenly md:flex-row md:gap-8">
				{(workCategories || []).map((workCategory) => {
					const myClients = clientCategories
						?.filter(
							(clientCategory) =>
								clientCategory.work_category_id ===
								workCategory.id,
						)
						?.toSorted((a, b) => (a.order || 0) - (b.order || 0));
					return (
						<div
							className="basis-0 md:basis-1/3 grow-0 md:w-1/3 mb-4"
							key={workCategory.id}
						>
							...
							<div className="text-md pl-4 md:pl-0 flex flex-col gap-1 md:gap-2">
								{(myClients || []).map((clientCategory) => (
									<Link
										href={"/clients/" + clientCategory.slug}
										className="max-w-44"
									>
										{clientCategory.name}
									</Link>
			...
			<div className="flex-row gap-8">
				...
				<div className="flex items-center justify-center gap-6">
					<div>
						...
						<Link
							href="/contact"
							className="flex items-center text-lg gap-4 md:ml-[32px] mt-2"
						>
							Contact Us <FaArrowRight />
						</Link>
						<div className="flex items-center text-lg gap-4 md:ml-[32px] mt-4 text-xs">
							© {new Date().getFullYear()} Telea Insights
						</div>
					</div>
				</div>
			</div>
